...
def test_notifier_sends_notification_by_default():
    ...
    # but patch.dict with empty dict and clear=True clears EVERYTHING which might break other
    # things?
    # Better to just patch the specific key we care about not being there.
    # But since we want to ensure it is NOT there, we can use patch.dict(os.environ) and del if
    # needed.
    # actually patch.dict(os.environ, {"KEY": "VAL"}) sets it. To unset, we might need verify it's
    # not there.
    ...
    with patch.dict(os.environ):
        ...
        # We need to mock sys.modules so 'import plyer' inside the function works and returns our
        # mock
        with patch.dict("sys.modules", {"plyer": mock_plyer}):
            Notifier.send("Title", "Message")

            mock_notification.notify.assert_called_once_with(
                title="Title", message="Message", app_name="Agent Pump", timeout=10
            )
