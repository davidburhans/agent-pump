...
from agent_pump.models.workspace import ProjectConfig, PhaseBackends, BackendInstance, BackendFallback, Workspace
...
def test_fallback_project_default(mock_workspace, project_config, mock_project):
    ...
    default_backend = BackendInstance(name="qwen", args=["--model", "qwen-2.5-coder"])
...
def test_get_backend_for_phase_logic(mock_get_backend, mock_workspace, project_config, mock_project):
    """Detailed logic test."""
    workflow = ProjectWorkflow(mock_project, mock_workspace, project_config=project_config)
    
    # 1. Test Fallback to Project Default
    # Setup default chain
    default_inst = BackendInstance(name="default_backend", args=["--arg1"])
    ...
    phase_inst = BackendInstance(name="phase_backend", args=["--arg2"])
    ...
    assert result == workflow.backend # Should be the default Gemini backend of the workflow instance
