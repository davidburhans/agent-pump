...
class TestCostCLICommands:
    ...
    def test_cost_show_workspace(self, cli_runner, mock_cost_service):
        ...
        with patch("agent_pump.models.workspace.Workspace.load") as mock_load:
            ...
            # Patch CostTrackingService in the cli module via sys.modules or just patch where it's used
            # Since the command imports it inside the function, we need to patch it there.
            with patch("agent_pump.services.cost_tracking_service.CostTrackingService") as MockService:
                MockService.return_value = mock_cost_service
            ...
            with patch("agent_pump.services.cost_tracking_service.CostTrackingService") as MockService:
                ...
                with patch.object(mock_cost_service, "export_costs", return_value="{}"):
                    ...
                    with cli_runner.isolated_filesystem():
                        result = cli_runner.invoke(main, ["cost", "export", "--output", "costs.json"])
    ...
    def test_cost_breakdown_by_phase(self, cli_runner, mock_cost_service):
        ...
        with patch("agent_pump.models.workspace.Workspace.load") as mock_load:
            ...
            with patch("agent_pump.services.cost_tracking_service.CostTrackingService") as MockService:
                ...
                with patch.object(mock_cost_service, "get_cost_breakdown_by_phase") as mock_breakdown:
                    # Use SimpleNamespace instead of MagicMock for data objects
                    mock_breakdown.return_value = {
                        "planning": SimpleNamespace(total_cost=0.02, total_tokens=1000, record_count=2),
                        "implementing": SimpleNamespace(total_cost=0.03, total_tokens=2000, record_count=3),
                    }
        ...
        with patch("agent_pump.models.workspace.Workspace.load") as mock_load:
            ...
            with patch("agent_pump.services.cost_tracking_service.CostTrackingService") as MockService:
                ...
                with patch.object(
                    ...
                    mock_breakdown.return_value = {
                        "gemini": SimpleNamespace(total_cost=0.05, total_tokens=3000, record_count=5),
                    }
    ...
    def test_cost_reset_nonexistent_project(self, cli_runner, mock_cost_service):
        ...
        with patch("agent_pump.models.workspace.Workspace.load") as mock_load:
            ...
            with patch("agent_pump.services.cost_tracking_service.CostTrackingService") as MockService:
                ...
                with patch.object(mock_cost_service, "reset_costs_for_project") as mock_reset:
                    # Provide 'y' input for confirmation
                    result = cli_runner.invoke(main, ["cost", "reset", "/nonexistent/project"], input="y")
