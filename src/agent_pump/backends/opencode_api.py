...
try:
    from opencode_ai import AsyncOpencode, APIError, APITimeoutError
...
"""
    ...
    async def run(
        ...
        try:
            ...
            logger.debug(f"Sending chat to session {session_id} with model {model_id}/{provider_id}")
            
            async with client.with_streaming_response.session.chat(
                id=session_id,
                model_id=model_id,
