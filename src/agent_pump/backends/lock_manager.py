...
class BackendLockManager:
    ...
    def get_lock(cls, key: str, limit: int = 1) -> asyncio.Semaphore | asyncio.Lock:
        ...
        """
        # If limit is 1, prefer Lock for strict key exclusivity
        # (and potential performance/debugging benefits)
        # But Semaphore(1) is functionally similar. Lock is often reentrant-safe
        # in some frameworks, but asyncio.Lock is not reentrant.
